<template>
  <transition name="fade">
    ...
    >
      ...
      >
        <div
            class="btn_back d-flex justify-start items-center cursor-pointer"
            @click="vModel = false"
        >
        ...
        <div class="tabs d-flex items-center justify-center">
          <div class="tab cursor-pointer">Test</div>
        </div>

        <div class="questions">
          <QuestionCircle
              v-for="num in $store.getters.getAllQuestions"
              :key="num"
              :circleNumber="num"
          />
        </div>
...
export default {
  ...
  computed: {
    vModel: {
      ...
      set(newValue) {
        this.$emit('update:showTest', newValue)
      }
    }
  }
}
